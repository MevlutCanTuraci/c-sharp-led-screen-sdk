...
namespace SDKLibrary
{
    ...
    public delegate void DeviceGeneralEvent(Device device, ResolveInfo msg);
    ...
    public class HDCommunicationManager : IDisposable
    {
        ...
        /// <summary>
        /// Resolved info report nesnesini yok eder. Bütün subscriberlarını yok eder.
        /// </summary>
        public void DisposeEvent()
        {
            Delegate[] subscribers = ResolvedInfoReport.GetInvocationList();
            foreach (Delegate subscriber in subscribers)
            {
                ResolvedInfoReport -= (ResolveInfoReportEventHandler)subscriber;
            }
        }
        ...
        /// <summary>
        /// 开始扫描局域网设备 (LAN cihazlarını taramaya başlayın)
        /// </summary>
        public void StartScanLANDevice(int dueTime = 1000, int period = 6000)
        {
            lock (scanLock)
            {
                if (_timerScan != null)
                ...
                _timerScan = new Timer(new TimerCallback(ScanTick!), this, dueTime, period);
            }
        }


        /// <summary>
        /// (LAN cihazlarını taramaya başlayın)
        /// </summary>
        public void EkranlariTara()
        {
            _udpClient = new UdpClient(0);
            _udpClient.EnableBroadcast = true;
            _udpClient.BeginReceive(UdpReceiveAsyncCallback, _udpClient);
            _timerScan = new Timer(new TimerCallback(ScanTick!), this, 5500, 10000);
        }
        ...
        /// <summary>
        /// 定时扫描设备函数函数 (Düzenli tarama cihazı işlevi işlevi)
        /// </summary>
        /// <param name="ar"></param>
        ...
        private void ScanDevice()
        {
            if (_udpClient != null)
            {
                ...
                //Console.WriteLine("Scan device içerisinde ScanDevice()");
            }
        }


        /// <summary>
        /// udp接收数据回调函数 (udp veri geri arama işlevi alır)
        /// </summary>
        /// <param name="ar"></param>
        private void UdpReceiveAsyncCallback(IAsyncResult ar)
        {
            //Console.WriteLine("UdpReceiveAsyncCallback içerisinde ");
            try
            {
        ...
        /// <summary>
        /// 解析下位机反馈的udp数据 (Alt bilgisayar tarafından geri beslenen udp verilerini analiz edin)
        /// </summary>
        ...
        /// <summary>
        /// 解析扫描指令 (Tarama Talimatlarını Ayrıştırma)
        /// </summary>
        /// <param name="srcData"></param>
        private void ResolveScanAnswer(byte[] srcData, IPEndPoint remoteEP)
        {
            ...
            deviceID = deviceID.Trim('\0'); // 去掉尾部的\0 (Sondaki \0'ı kaldır)
                                            // 当前设备不存在则添加到Devices (Mevcut cihaz yoksa, onu Cihazlar'a ekleyin)
            bool found = false;
            lock (_deviceLock)
            {
            ...
            if (!found)
            {
                ...
                string remoteIP = remoteEP.Address.ToString();
                {
                    ...
                    if (device != null)
                    {
                        ...
                        Devices.Add(device);
                        AllDevices.Add(new Tuple<Device, string>(device, $"{remoteIP}"));
        ...
        /// 客户端会话列表
        /// </summary>
        public List<Device> Devices { get;  set; } //343


        #region Configuration yapılacağı zaman kullanılır


        /// <summary>
        /// Device sınıfı bir de ip adresi alır.
        /// </summary>
        public List<Tuple<Device, string>> AllDevices { get; set; } = new List<Tuple<Device, string>>(); //344
        ...
        public Tuple<string, string> DeviceIp { get; set; }
        ...
        /// <summary>
        /// 与客户端的连接已建立事件 (İstemci ile bağlantı kuruldu olayı)
        /// </summary>
        ...
        private Device AddDevice(string ip, int port, out string exception)
        {
            ...
            try
            {
                ...
                {
                    //sonradan eklendi.
                    if (TargetDeviceID == device.GetDeviceInfo().deviceID)
                    {
                        Devices.Add(device);
                    }

                    // 开始异步接收数据，并主动触发RaiseClientConnected 函数初始化协议版本 (Eşzamansız olarak veri almaya başlayın ve protokol sürümünü başlatmak için RaiseClientConnected işlevini aktif olarak tetikleyin)
        ...
        //Deneme amaçlı eklendi. Sonradan eklendi.
        /// <summary>
        /// Instance'ı çoğaltmak için oluşturulmuş bir fonksiyondur. Gelen ip adresini kullanarak cihaz ekler.
        /// Deneme amaçlı sonradan eklenmiştir!
        /// </summary>
        /// <param name="ip">Device id - Bu değeri almak için Hd CM içerisinden DeviceIp tuple içerisinde mevcuttur.</param>
        /// <returns>Eklenen cihaz'a ait sınıfı döner.</returns>
        public Device AddDevice222(string ip)
        {
            string exception = "";
            try
            {
                TcpClient client = new TcpClient(ip, 10001);
                Device device = new Device(this, client);
        ...
        /// <summary>
        /// 定时检查发送超时状态与自动发送数据 (Gönderme zaman aşımı durumunu düzenli olarak kontrol edin ve verileri otomatik olarak gönderin)
        /// </summary>
        ...
        /// <summary>
        /// 处理客户端连接的函数 (İstemci bağlantılarını yöneten işlevler)
        /// </summary>
        /// <param name="ar"></param>
        ...
        /// <summary>
        /// 数据接受回调函数 (Veri geri arama işlevini kabul eder)
        /// </summary>
        /// <param name="ar"></param>
            ...
            try
            {
                lock (_deviceLock)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Close(device);
                            //触发客户端连接断开事件 (İstemci bağlantısının kesilmesi olayını tetikle)
                    ...
                    catch (Exception exp)
                    {
                        ...
                        device.EndToSend(); // Burası kapatıldı. Geri açılabilir ya da açılmaya bilir.
                    }
        ...
        /// <summary>
        /// 异步发送数据至指定的客户端 (Verileri belirtilen istemciye eşzamansız olarak gönder)
        /// </summary>
        /// <param name="device">接收数据的客户端会话 (Veri almak için müşteri oturumu)</param>
        /// <param name="data">数据报文 (veri birimi)</param>
        ...
        /// <summary>
        /// 触发客户端连接事件 (İstemci bağlantı olayını tetikle)
        /// </summary>
        /// <param name="state"></param>
        private void RaiseClientConnected(Device device)
        {
            // 发送版本协商协商命令 (Sürüm anlaşması müzakere komutunu gönder)
            device.SendEnsureProtocolVersionCmd();

            // 发送SDK版本协商命令 (SDK sürüm anlaşması komutunu gönder)
        ...
        /// <summary>
        /// 接收到数据事件 (veri olayı alındı)
        /// </summary>
        public event EventHandler<AsyncEventArgs> DataReceived;



        //public List<Tuple<Device, ResolveInfo>> ImageResult = new List<Tuple<Device, ResolveInfo>>();
        public List<string> ImageResult          = new List<string>();
        public List<string> SendProgramResult    = new List<string>();
        public List<string> CurrentProgramResult = new List<string>();
        // Burası cihazdan gelen verileri event'lere gönderen fonksiyon. SDK ORJ
        public void RaiseDataReceived(Device device)
        {
            ...
            while (true)
            {
                ...
                // 如果当前接收的数据比命令包长度还要短，直接退出继续接收数据 (Halihazırda alınan veri, komut paketinin uzunluğundan daha kısaysa, doğrudan çıkın ve veri almaya devam edin.)
                ...
                // 处理TCP粘包的情况，把后面的包放到MemBufStream前面 (TCP yapışkan paketlerinin durumunu yönetin, sonraki paketleri MemBufStream'in önüne koyun)
                if (nCmdPackLen <= device.Client.MemBufStream.Length)
                {
                ...
                //Döndürülen xml'nin toplam verilerinin en büyük tek veri paketinden daha büyük olduğu durumla başa çıkmak için
                if (cmd == CmdType.kSDKCmdAnswer)
                {
                    device.ResolveTcpReturnData(commanddata, out ri);
                    // xml数据还没有完全接收，退出不反馈 (Xml verileri tam olarak alınmadı, geri bildirim olmadan çıkın)
                    if (device.SDKCmdAnswerXmlData != null)
                    {
                        //ConsoleEx.WriteLine($"Xml verileri tam olarak alınmadı, geri bildirim olmadan döngüden çıkılıyor. -> cmd == CmdType.kSDKCmdAnswer in device.SDKCmdAnswerXmlData != null if action is true.", ConsoleColor.Red);
                        break;
                    }
                ...
                //Eğer gelen xml verisi boş değilse, üsste tanımlanan ResolveInfo ri içerisine atıyoruz.
                else
                {
                    device.ResolveTcpReturnData(commanddata, out ri);
                }
                ...
                {
                    //Console.WriteLine("ResolvedInfoReport içerisinde doluymuşşş");
        ...
        /// <summary>
        /// 发送数据前的事件 (Veri gönderilmeden önceki olay)
        /// </summary> 
        ...
        /// <summary>
        /// 数据发送完毕事件 (veri gönderme olayı)
        /// </summary>
        public event EventHandler<AsyncEventArgs> CompletedSend;

        /// <summary>
        /// 触发数据发送完毕的事件 (Verilerin gönderildiği olayı tetikleyin)
        /// </summary>
        /// <param name="state"></param>
        ...
        /// <summary>
        /// 网络错误事件 (ağ hatası olayı)
        /// </summary>
        public event EventHandler<AsyncEventArgs> NetError;
        /// <summary>
        /// 触发网络错误事件 (Ağ hatası olayını tetikle)
        /// </summary>
        /// <param name="state"></param>
