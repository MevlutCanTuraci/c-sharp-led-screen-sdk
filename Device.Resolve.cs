...
namespace SDKLibrary
{
    partial class Device
    {
       ...
       /// </summary>
        ...
        /// <summary>
        /// 解析返下位机回来的tcp数据 (Alt bilgisayardan dönen tcp verilerini analiz edin)
        /// </summary>
        ...
        public ErrorCode ResolveTcpReturnData(byte[] srcData,  out ResolveInfo ri)
        {
            ...
            switch (cmd)
            {
                ...
                case CmdType.kFileStartAnswer:  // 文件开始上传请求应答 (Dosya başlatma yükleme isteği yanıtı)
                    {
                ...
                case CmdType.kFileEndAnswer:  // 结束文件开始上传请求应答 (dosya sonu yükleme isteği yanıtını başlat)
                    {
                        HasStartSendFileContext = false;
                        SendingFile = false;
                        if (srcData.Length >= 6)
                        {
                            ...
                            if (state == (int)ErrorCode.kSuccess)
                            {
                                ...
                                {
                                    //Sonradan eklendi
                                    CommunicationManager.ImageResult.Add($"{error}");
                ...
                case CmdType.kReadFileAnswer:   // 回读文件应答 (geri okuma dosyası yanıtı)
                    {
                        DowningFile = false;
                ...
                case CmdType.kFileEndAsk:   // 文件下载结束请求 (Dosya indirme bitiş isteği)
                    {
                        error = ErrorCode.kSuccess;
